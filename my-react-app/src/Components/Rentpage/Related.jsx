...
const allPagesData = [
  {
    ...
    side: { big: sideimage, 
      small: [sideimage, sideimage, sideimage],
       title: "Apartment",
        beds: 4, 
        baths: 1,
        sqft: 400,
  ...
  },
  {
    ...
    side: { big: sideimage, small: [sideimage, sideimage, sideimage],
       title: "Villa",
        beds: 5,
         baths: 3,
          sqft: 800,
...
const Related = () => {
  ...
  return (
    <div className="container py-5">
      <h3 className={`mb-4 text-center ${styles.heading}`}>Related Properties</h3>
            <p className={`text-center mb-4 ${styles.subheading}`}>
  Explore similar homes that match your style and budget
      ...
      <div className="row g-4">
        ...
        <div className="col-lg-5">
          <div className={`${styles.sideBigImgWrapper} ${styles.rotateAnimation}`} key={currentPage}>
            <img src={pageData.side.big} className={`img-fluid rounded-4 mb-3 ${styles.bigImg}`} />
          </div>

          <div className="row g-3 mb-3">
            {pageData.side.small.map((img, idx) => (
              <div className="col-4" key={idx}>
                <img src={img} className={`img-fluid rounded-3 ${styles.smallImg}`} />
              </div>
            ))}
          </div>

          <div>
            <h6 className={styles.titleRight}>{pageData.side.title}</h6>
            ...
            <button className={styles.buttonsss} onClick={() => navigate("/contact")}>
              Buy Now
            </button>
          </div>
        </div>
      ...
      <div className="d-flex justify-content-center align-items-center gap-3 mt-5">
        <button className={styles.page} onClick={handlePrev}>←</button>
        {allPagesData.map((_, idx) => (
          <button
            key={idx}
            className={`${styles.page} ${currentPage === idx ? styles.active : ""}`}
            onClick={() => handlePageClick(idx)}
          >
            {idx + 1}
          </button>
        ))}
        <button className={styles.page} onClick={handleNext}>→</button>
      </div>
    </div>
  );
};
